...
pub fn execute_player_spawn(
    ...
    let center = if game.config.random_spawn { None } else { tile };

    let Some(spawn) = find_spawn(game, center, random) else {
        return;
    };
...
pub fn get_spawn_tiles(
    ...
    // TS `getSpawnTiles`: owner and land only (no impassable concept).
