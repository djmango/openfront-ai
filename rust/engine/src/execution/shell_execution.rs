...
impl ShellExecution {
    ...
    /// TS `getEffectOnTargetForTesting()` - reads the random damage roll directly rather than
    /// through a full `hit_target()` call. Requires `init()` to have run first (so
    /// `damage_random` is seeded), same as TS's init requirement before calling
    /// `effectOnTarget()`.
    ...
    fn effect_on_target(&mut self, game: &Game) -> i32 {
        let roll = self
            ...
            .unwrap_or(1);
        let damage_multiplier = (roll - 1) * 25 + 200;
    ...
    fn hit_target(&mut self, game: &mut Game) {
        ...
        if destroyed {
            ...
            // Kept as an inert hook: tip OpenFront no longer grants warship veterancy, but this
            // preserves the native call boundary for stats/transport-kill parity plumbing.
...
// Tip OpenFront no longer has `WarshipVeterancy.test.ts`; shell damage is just the
// per-shot random roll scaled by base shell damage. These tests ensure stale native
// veterancy fields cannot change that formula.
#[cfg(test)]
mod tests {
    use super::*;
    ...
    #[test]
    fn shell_damage_ignores_stale_firing_warship_veterancy() {
        ...
        let stale_vet_shooter = game.build_unit(1, WARSHIP, game.map.ref_xy(1, 0));
        game.unit_mut(1, stale_vet_shooter).unwrap().veterancy = 3;
        ...
        for tick in 1..30u32 {
            ...
            let mut stale_vet_shell =
                ShellExecution::new(game.map.ref_xy(1, 0), 1, stale_vet_shooter, 2, target);
            base_shell.init(&mut game, tick);
            stale_vet_shell.init(&mut game, tick);

            let d_base = base_shell.get_effect_on_target_for_testing(&game);
            let d_stale_vet = stale_vet_shell.get_effect_on_target_for_testing(&game);

            assert_eq!(d_stale_vet, d_base);
        }
    }

    #[test]
    fn a_shell_landing_the_killing_blow_does_not_award_veterancy() {
        let mut game = crate::test_util::plains_game(20, 20);
        add_player(&mut game, 1);
        add_player(&mut game, 2);
        ...
        assert_eq!(game.unit(1, shooter).unwrap().veterancy, 0);
        assert_eq!(game.unit(1, shooter).unwrap().veterancy_progress, 0);
        assert_eq!(game.unit_veterancy(1, shooter), 0);
    }

    // Ported from `openfront/tests/ShellRandom.test.ts`. These exercise the underlying PRNG draw:
