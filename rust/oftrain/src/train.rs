...
mod v81_state_and_gate_tests {
    ...
    fn resume_requires_matching_schedule_identity() {
        ...
        assert!(
            reconcile_resume_schedule(&mut legacy, CurriculumSchedule::V811, true).is_err()
        );
    }
        ...
        for result in &next_pending {
            for info in &result.ep_infos {
                ...
                if let Err(e) = metrics.log(&serde_json::json!({
                    ...
                    "reward": info.reward,
                    "episode/length": info.length,
                    "episode/final_tiles": info.final_tiles,
                    "episode/final_tick": info.final_tick,
                    "episode/place": info.place,
                    "episode/players": info.n_players,
                    "episode/score": info.score,
                    "episode/won": info.won,
                    "episode/wasted": info.wasted,
