...
def validate_manifest_bytes(body: bytes) -> dict[str, Any]:
    ...
    if schema == 2:
        ...
        for positive in ("hidden_size", "context_hidden", "bptt_length"):
            if not isinstance(recurrent.get(positive), int) or recurrent[positive] <= 0:
                raise ValueError(f"invalid recurrent {positive}")
        if not isinstance(recurrent.get("context_schema"), str):
            raise ValueError("invalid recurrent context_schema")
    return manifest
