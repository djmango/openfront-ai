 ...
 *
 * Checks out `openfront/` to the record's engine commit (from path bucket or
 * record.gitCommit) before replay - required for hash parity.
 *
 * Usage: tsx scripts/hash_verify.ts path/to/record.json.gz
 */
...
async function main() {
  ...
  if (!file) {
    throw new Error("usage: hash_verify.ts <record.json>");
  }
  ...
  const commit =
    ...
    null;
  if (!commit) {
    throw new Error("no engine commit on record or path");
  }
  const restore = pinEngine(commit);
  try {
    const gameId = record.info.gameID;
    const outDir = `/tmp/of-hash-verify-${gameId}`;
